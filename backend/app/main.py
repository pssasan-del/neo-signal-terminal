...
async def runtime_error_handler(request: Request, exc: RuntimeError):
    code=str(exc)
    if code == "KOTAK_SESSION_EXPIRED":
        execution_engine.arm(False)
        risk_engine.set_enabled(False)
        return JSONResponse(status_code=401, content={"detail":"KOTAK_SESSION_EXPIRED","action":"LOGIN_REQUIRED"})
    return JSONResponse(status_code=409, content={"detail": code})
...
@app.post("/signals/lifecycle/{signal_id}/resolve-trade")
async def lifecycle_resolve_trade(signal_id: str):
    """Backfill broker contract metadata for old/new stock signals before opening the trade ticket."""
    item = signal_lifecycle.items.get(signal_id)
    if item is None: raise HTTPException(404, "Signal not found")
    if item.trading_symbol:
        return {"ok": True, "signal": item.to_dict()}
    meta = broker.signal_trade_meta.get(item.symbol_key)
    if not meta and item.symbol_key.startswith("nse_cm|"):
        symbol = item.symbol_key.split("|", 1)[1]
        if symbol.upper().replace(" ", "") not in {"NIFTY50", "NIFTYBANK", "BANKNIFTY"}:
            resolved = await scanner_controller._resolve_one(symbol)
            if resolved is not None:
                meta = {"trading_symbol": resolved.trading_symbol, "exchange_segment": resolved.exchange_segment, "instrument_token": resolved.instrument_token}
                broker.signal_trade_meta[item.symbol_key] = meta
    if meta:
        item.trading_symbol = meta.get("trading_symbol")
        item.exchange_segment = meta.get("exchange_segment")
        item.instrument_token = meta.get("instrument_token")
        item.trade_ready = True
        item.option_status = item.option_status or "READY"
        item.updated_at = time.time()
        signal_lifecycle._save(item)
        return {"ok": True, "signal": item.to_dict()}

    # For core indices, resolve an actual live option contract now. Never fabricate a cash-index order symbol.
    key=item.symbol_key.upper().replace(' ','')
    if any(x in key for x in ('NIFTY','BANK','SENSEX')):
        opt=await instruments.auto_select_index_option(symbol_key=item.symbol_key, side=item.side, underlying_ltp=float(item.last_price or item.entry))
        selected=opt.get('selected') if isinstance(opt,dict) else None
        if opt.get('status')=='READY' and isinstance(selected,dict):
            item.trading_symbol=selected.get('trading_symbol')
            item.exchange_segment=selected.get('exchange_segment')
            item.instrument_token=selected.get('instrument_token')
            item.trade_ready=True; item.option_status='READY'
            item.underlying_symbol=opt.get('underlying'); item.option_type=selected.get('option_type')
            item.strike=selected.get('strike'); item.expiry=selected.get('expiry') or opt.get('expiry')
            item.option_ltp=selected.get('ltp'); item.option_bid=selected.get('bid'); item.option_ask=selected.get('ask')
            item.option_volume=selected.get('volume'); item.option_oi=selected.get('oi'); item.option_oi_change=selected.get('oi_change')
            item.option_iv=selected.get('iv'); item.option_delta=selected.get('delta'); item.option_gamma=selected.get('gamma')
            item.option_theta=selected.get('theta'); item.option_vega=selected.get('vega'); item.option_quality_score=selected.get('score'); item.option_lot_size=selected.get('lot_size')
            item.updated_at=time.time(); signal_lifecycle._save(item)
            return {'ok':True,'signal':item.to_dict(),'option':opt}
        item.trade_ready=False; item.option_status=str(opt.get('reason') or 'OPTION_DATA_UNAVAILABLE'); item.updated_at=time.time(); signal_lifecycle._save(item)
        return {'ok':False,'reason':item.option_status,'signal':item.to_dict(),'option':opt}

    return {"ok": False, "reason": "NO_TRADABLE_CONTRACT", "signal": item.to_dict()}
...
async def system_diagnostics():
    age = None if broker.last_market_message_at is None else max(0.0, time.time() - broker.last_market_message_at)
    return {
        "authenticated": broker.authenticated,
        "market_task_running": bool(broker.market_task and not broker.market_task.done()),
        "order_task_running": bool(broker.order_task and not broker.order_task.done()),
        "market_message_age_sec": age,
        "subscriptions": len(broker.subscriptions),
        "scanner": scanner_controller.status(),
        "candle_storage": candles.storage_summary(300),
        "signal_stats": signal_lifecycle.stats(),
        "live_orders_unlocked": settings.live_orders_unlocked,
        "risk": risk_engine.snapshot(),
...
async def lifecycle_list(limit: int = 100): return signal_lifecycle.list(limit)
...
async def evaluate_signal(body: SignalEvalRequest):
    ...
    if body.closed_only and rows:
        ...
        rows = rows[:-1]
    if body.timeframe_sec == 300:
        return signal_engine.evaluate_strongest(
            body.symbol_key, rows,
            rows_1m=candles.get_history(body.symbol_key, 60, 100)[:-1] if body.closed_only else candles.get_history(body.symbol_key, 60, 100),
            rows_15m=candles.get_history(body.symbol_key, 900, 100)[:-1] if body.closed_only else candles.get_history(body.symbol_key, 900, 100),
            rows_4h=candles.get_history(body.symbol_key, 14400, 30)[:-1] if body.closed_only else candles.get_history(body.symbol_key, 14400, 30),
            rows_1d=candles.get_history(body.symbol_key, 86400, 15)[:-1] if body.closed_only else candles.get_history(body.symbol_key, 86400, 15),
        )
...
async def create_exit_plan(body: ExitPlanRequest):
    if body.auto_exit and (not risk_engine.config.trading_enabled or not execution_engine.armed or not settings.live_orders_unlocked):
        raise HTTPException(409, "Auto-exit requires Risk ON, Execution ARMED and live-order server gate unlocked")
    try:
...
async def holdings():
    if not broker.authenticated: raise HTTPException(401, "Login required")
    return await broker.holdings_enriched()
...
@app.get("/execution")
async def execution_state(): return {**execution_engine.snapshot(), "live_orders_unlocked": settings.live_orders_unlocked, "risk_trading_enabled": risk_engine.config.trading_enabled}
...
async def execution_intent(body: ExecutionIntentRequest):
    ...
    result = execution_engine.create_intent(order=order, symbol=body.trading_symbol, side=side,
        ...
        open_positions=int(summary.get("open_positions") or 0), day_pnl=float(summary.get("day_mtm") or 0.0), signal_id=body.signal_id)
    if result.get("ok"):
...
async def execution_confirm(intent_id: str, body: ExecutionConfirmRequest):
    ...
    result = await execution_engine.confirm(intent_id=intent_id, confirmation_token=body.confirmation_token,
        live_price=body.live_price, open_positions=int(summary.get("open_positions") or 0), day_pnl=float(summary.get("day_mtm") or 0.0),
    ...
    try:
        while True:
            try:
                ...
                payload = {"channel":"heartbeat","server_time":time.time()}
