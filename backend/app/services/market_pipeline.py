...
class CandleBuilder:
    ...
    def __init__(self, timeframes: tuple[int, ...] = (60, 180, 300, 900, 14400, 86400)) -> None:
    ...
    def storage_summary(self, timeframe_sec: int = 300) -> dict[str, Any]:
        """Return persisted closed-candle counts without including forming candles."""
        with sqlite3.connect(self.db_path) as con:
            rows = con.execute(
                """
                SELECT symbol_key, COUNT(*), MIN(start_ts), MAX(start_ts)
                FROM candles
                WHERE timeframe_sec = ?
                GROUP BY symbol_key
                ORDER BY symbol_key
        ...
        items = [
            {
                "symbol_key": row[0],
                "closed_count": int(row[1]),
                "first_ts": row[2],
                "last_ts": row[3],
                "ready": int(row[1]) >= 24 if int(timeframe_sec) == 300 else None,
            }
            for row in rows
        ]
        return {
            ...
            "symbols": len(items),
            "total_closed": sum(x["closed_count"] for x in items),
            "ready_symbols": sum(1 for x in items if x.get("ready") is True),
            "items": items,
        }
